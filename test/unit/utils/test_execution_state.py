...
class TestExecutionState:
    ...
    class TestDelay:
        ...
        class TestDelayChangedAfterFirstExecution:
            @before
            def before(self):
            ...
            def test_we_then_waited_for_default_delay(self):
                is_time_to_execute = self.state.wait_for_next_tick_or_stop()
                self.mock_queue.get.assert_called_once_with(block=True, timeout=2.7)
                assert is_time_to_execute

        class TestDelayChangedDuringAnExecution:
            @before
            def before(self):
                self.default_delay = datetime.timedelta(milliseconds=2700)
        ...
        class TestDelayProviderChangedDuringAnExecution:
            @before
            def before(self):
                self.default_delay = datetime.timedelta(milliseconds=2700)
    ...
    class TestPausedTime:
        ...
        class TestWhenPausedDuringInitialDelay:
            def test_we_waited_for_remaining_time_after_resume(self):
                is_time_to_execute = self.state.wait_for_next_tick_or_stop()
            ...
            class TestWhenPausedAgainAfterFirstExecution:
                @before
                def before(self):
                ...
                def test_we_waited_for_correct_remaining_time_after_second_resume(self):
        ...
        class TestWhenPausedMultipleTimesInInitialDelay:
            @before
            def before(self):
                self.first_pause_time = 3
                self.first_resume_time = 8
            ...
            def test_the_wait_time_accumulates(self):
                is_time_to_execute = self.state.wait_for_next_tick_or_stop()
                self.mock_queue.get.assert_has_calls([
                    call(block=True, timeout=10),   # wanted to wait for 10s, the initial delay
                    call(block=True),               # after pause at 3s, wait until next resume
        ...
        class TestWhenPausedMultipleTimesInNormalDelay:
            @before
            def before(self):
