...
from codeguru_profiler_agent.profiler_disabler import KillSwitch, CpuUsageCheck, ProfilerDisabler, MINIMUM_SAMPLES_IN_PROFILE
...
class TestProfilerDisabler:
    ...
    class TestCpuUsageCheck:
        ...
        class TestGetAverageSamplingIntervalSeconds:
            @before
            def before(self):
            ...
            class TestWhenProfileIsNone:
                def test_it_returns_configuration_sampling_interval(self):
                    assert CpuUsageCheck._get_average_sampling_interval_seconds(None) == 23

            class TestWhenProfileSampleCountLessThanMinSamplesInProfile:
                def test_it_returns_configuration_sampling_interval(self):
        ...
        class TestIsCpuUsageLimitReached:
            @before
            def before(self):
                with patch(
            ...
            class TestWhenAverageDurationExceedsLimit:
                def test_it_returns_true(self):
                    # timer: (0.5/4) * 100= 12.5%
                    assert self.process_duration_check.is_cpu_usage_limit_reached()

            class TestWhenAverageDurationIsBelowLimit:
                def test_it_returns_false(self):
                    # timer: (0.5/4) * 100= 12.5%
            ...
            class TestWhenProfileIsNone:
                def test_it_calls_get_average_sampling_interval_without_profile(self):
        ...
        class TestWhenTimerDoesNotHaveTheKey:
            def test_it_returns_false(self):
                self.process_duration_check.timer = Timer()
                assert not self.process_duration_check.is_cpu_usage_limit_reached()

        class TestWhenTimerDoesNotHaveEnoughMeasures:
            def test_it_returns_false(self):
                self.timer.reset()
                for i in range(4):
                    self.timer.record('runProfiler', 0.5)
    ...
    class TestMemoryLimitCheck:
        ...
        class TestWhenMemoryUsageExceedsLimit:
            def test_it_returns_true(self):
                assert self.disabler._is_memory_limit_reached(self.profile)

        class TestWhenMemoryUsageIsBelowLimit:
            def test_it_returns_false(self):
                self.profile.get_memory_usage_bytes = Mock(return_value=10 * 1024 * 1024 - 1)
